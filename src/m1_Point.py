...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        self.dist_traveled = self.dist_traveled + abs(self.get_distance_from(p))
    ...
    def get_distance_from(self,another_point):
        xc = self.x - another_point.x
        yc = self.y - another_point.y
        return (((xc ** 2) + (yc ** 2)) ** .5)

    def get_distance_from_start(self):
        p_ori = Point(self.ori_x,self.ori_y)
        val = self.get_distance_from(p_ori)
        return val
    ...
    def closer_to(self,p2,p3):
        if abs(self.get_distance_from(p2)) <= abs(self.get_distance_from(p3)):
            return p2
        else:
            return p3
